...
그 파일에 관한 정보를 받아서, 다음 controller(우리는 postEdit) 에 전달해 주는거다. 
...
자 이제 드디어 비디오를 업로드 해볼건데 그러려면 비디오파일이 당연히 필요하겠지다. 
...
그리고 내가 준비한 파일을 이제 업로드해주고 "Upload Video" 하기전에 잠깐 videoRouter 수정할게 있다. 
...
그리고 multer를 사용해서 파일을 업로드하고 싶다면 form의 encoding type 바꿔주는거 잊지 마삼!!
...
그래서 우린 이 mongoose에게 이 owner가 어떤 model의 ObjectId 인지 알려줄거다. 
...
일단 컨트롤러에서 비디오 올린 user의 정보를 video.owner로 부터 가져온뒤 
